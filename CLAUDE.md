...
**REQ_FOLLOW address is MSB-first (human-readable order).** The Nordic
SnifferAPI reverses OTA address bytes (LSB-first) to MSB-first before sending
REQ_FOLLOW, so the firmware expects `04 E3 E5 B0 87 05` for address
`04:E3:E5:B0:87:05`. This matches the human-readable string order.

**REQ_FOLLOW suppresses ADV_IND delivery.** Once follow mode is active, the
firmware delivers very few advertising packets from the target; it is primarily
waiting for CONNECT_IND. Do not use `--follow` for advertising-payload captures
